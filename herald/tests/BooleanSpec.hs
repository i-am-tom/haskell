...
spec_boolean_basic_bad = test Rejection
  { _input     = Number 123
...
spec_boolean_basic_good = test Acceptance
  { _input  = Bool True
  , _schema = boolean $ pure ()
  }

spec_boolean_enum_bad :: Spec
spec_boolean_enum_bad = test Rejection
  { _input     = Bool True
...
spec_boolean_enum_good = test Acceptance
  { _input  = Bool False
  , _schema = boolean $ enum [(False, ())]
  }
