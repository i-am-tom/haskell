...
instance
  (Generic x, Monoid (Γ (Rep x) Void), GChange (Rep x)) =>
  Change (Generically x)
  where
...
deriving instance
  (Eq (x v), Eq (Γ x v), Eq (y v), Eq (Γ y v)) =>
  Eq (Choice x y v)

deriving instance
  (Ord (x v), Ord (Γ x v), Ord (y v), Ord (Γ y v)) =>
  Ord (Choice x y v)

deriving instance
  (Show (x v), Show (Γ x v), Show (y v), Show (Γ y v)) =>
  Show (Choice x y v)
